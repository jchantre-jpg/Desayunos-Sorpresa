...
│   ├── firebase-products.js # CRUD productos (Firestore + Storage)
...
2. **Productos:**  
   - **Con Firebase:** Configura Firebase (ver `FIREBASE_SETUP.md`), activa `USE_FIREBASE` en `js/firebase-config.js` y usa el **panel de administración** (`admin.html`). Inicia sesión (usuario: `admin`, contraseña: `admin123` por defecto) para agregar productos, subir fotos, descripción, contenido del detalle y precios.  
   - **Sin Firebase:** Modifica el array `PRODUCTOS` en `js/products.js`.
